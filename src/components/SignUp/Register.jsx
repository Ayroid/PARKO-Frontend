  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://parko.studio:3000/user/register",
        formData
      );
      ...
      <div className="font-Nunito">
        <div className="flex flex-col items-center gap-5 pt-5">
          ...
          <p className="w-3/4 text-center">
            Lorem ipsum is impy dummy test of the printing and typesitting
          </p>
        </div>
          ...
          <div className="bg-orange-500 rounded-tl-xl rounded-tr-xl p-5">
            <form className="text-lg" onSubmit={handleSubmit}>
              ...
              <div className="p-5">
                <button
                  type="submit"
                  className="w-full py-2 text-gray-700 border rounded-md bg-yellow-400 focus:bg-yellow-500"
                >
                  Sign Up
                </button>
              </div>
    ...
    <div className="mb-4">
      <label
        ...
        className={`absolute transform ${
          value ? "-translate-y-2 text-xs text-gray-600" : "text-base"
        } transition-transform duration-300 px-3 py-2`}
      >
        {label}
      </label>
      <input
